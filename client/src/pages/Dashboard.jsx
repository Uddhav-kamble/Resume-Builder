import { FilePenLineIcon, LoaderCircleIcon, PencilIcon, PencilLine, PlusIcon, TrashIcon, UploadCloud, UploadCloudIcon, XIcon } from 'lucide-react'
...
const Dashboard = () => {
  ...
  const uploadResume = async (event) => {
    event.preventDefault()
    setIsLoading(true)
    console.log('Selected resume file:', resume)
    try {
      if (!resume) {
      ...
      if (!resumeText || resumeText.trim() === '') {
        // resumeText = "abcd"
        toast.error('Could not extract text from PDF. Please ensure the PDF contains selectable text.')
        setIsLoading(false)
        return
      }
  ...
  return (
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <p className='text-2xl font-medium mb-6 bg-linear-to-r from-slate-600 to-slate-700 bg-clip-text
        text-transparent sm:hidden'>Welcome, Joe Doe</p>

        <div className='flex gap-4'>
          <button onClick={() => setShowCreateResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg
          gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-indigo-500 hover:shadow-lg
          transition-all duration-300 cursor-pointer'>
            <PlusIcon className='size-11 transition-all duration-300 p-2.5 bg-linear-to-br from-indigo-300 to-indigo-500
          ...
          transition-all duration-300 cursor-pointer'>
            <UploadCloudIcon className='size-11 transition-all duration-300 p-2.5 bg-linear-to-br from-purple-300 to-purple-500
            text-white rounded-full'/>
            ...
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <div>
                <label htmlFor="resume-input" className='block text-sm text-slate-700'>
                  Select resume file
                  <div className='flex flex-xol items-center justify-center gap-2 border group text-slate-400 border-dashed rounded-mdp-4 py-10 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colors'>
                    {resume ? (
              ...
              <button disabled={isLoading} className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-2'>
